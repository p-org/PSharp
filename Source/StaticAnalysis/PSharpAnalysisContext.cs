...
namespace Microsoft.PSharp.StaticAnalysis
{
    ...
    public sealed class PSharpAnalysisContext : AnalysisContext
    {
        ...
        /// <param name="model">SemanticModel</param>
        /// <returns>Boolean</returns>
        internal override bool IsMachineType(IdentifierNameSyntax identifier, SemanticModel model)
        {
            TypeInfo typeInfo;
            try
            {
            ...
            if (typeInfo.Type != null && typeInfo.Type.ToString().Equals(typeof(Machine).FullName))
            {
                return true;
            }
            else
            {
                ...
                if (definition != null)
                {
                    ...
                    {
                        string machineName = base.GetFullClassName(machineNode as ClassDeclarationSyntax);

                        foreach (var knownMachine in this.Machines)
                        {
                            string knownMachineName = base.GetFullClassName(knownMachine);
