...
namespace Microsoft.PSharp.Runtime
{
    ...
    /// </summary>
    internal sealed class NekaraRuntime : MachineRuntime
    {
        public static ITestingService NekaraApi = RuntimeEnvironment.Client.Api;
        /*public static Nekara.Helpers.UniqueIdGenerator TaskIdGenerator = RuntimeEnvironment.Client.TaskIdGenerator;
        public static Nekara.Helpers.UniqueIdGenerator ResourceIdGenerator = RuntimeEnvironment.Client.ResourceIdGenerator;*/
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="NekaraRuntime"/> class.
        /// </summary>
        internal NekaraRuntime()
            : this(Configuration.Create())
        {
            this.Configuration.EnableMonitorsInProduction = true;
        }

        /// <summary>
        /// Initializes a new instance of the <see cref="NekaraRuntime"/> class.
        /// </summary>
        internal NekaraRuntime(Configuration configuration)
            : base(configuration)
        {
            this.Configuration.EnableMonitorsInProduction = true;
            ...
            Machine creator, Guid opGroupId)
        {
            Machine machine = this.CreateMachine(mid, type, machineName, creator, opGroupId);
            this.LogWriter.OnCreateMachine(machine.Id, creator?.Id);
            Nekara.Models.Task.Run(() =>
            {
                this.RunMachineEventHandlerAsync(machine, e, true).Wait();
            }).Wait();
        ...
        {
            // Console.WriteLine("\n>>> Creating Machine : {0} {1}", type.Name, machineName);
            ...
            Guid opGroupId, SendOptions options)
        {
            ...
            if (enqueueStatus is EnqueueStatus.EventHandlerNotRunning)
            {
                Nekara.Models.Task.Run(() =>
                {
                    this.RunMachineEventHandlerAsync(targetMachine, null, false).Wait();
                }).Wait();
                // await this.RunMachineEventHandlerAsync(targetMachine, null, false);
                return Task.FromResult(true);
            }

            return Task.FromResult(enqueueStatus is EnqueueStatus.Dropped);
        }
        ...
        {
            // Console.WriteLine("\n>>> Enqueing Event : {2} >>> {0} -> {1}", sender.Id.ToString(), target.ToString(), e.ToString());
            NekaraApi.ContextSwitch();
        ...
        {
            // Console.WriteLine("\n>>> Running Machine Event Handler: {0}, {1}", machine.GetType().Name, initialEvent != null ? initialEvent.GetType().Name : string.Empty);
            Nekara.Models.Task.Run(() =>
            {
                try
                {
                    if (isFresh)
                    {
                        machine.GotoStartState(initialEvent).Wait();
                    }

                    machine.RunEventHandlerAsync().Wait();
                }
                catch (Exception ex)
                {
                    this.IsRunning = false;
                    this.RaiseOnFailureEvent(ex);
                    if (ex is NekaraException || ex is TaskCanceledException)
                    {
                        throw;
                    }
                }
        ...
        /// Runs a new asynchronous machine event handler.
        /// </summary>
        private Task RunMachineEventHandlerAsync(Machine machine, Event initialEvent, bool isFresh)
        {
            // Console.WriteLine("\n>>> Running Machine Event Handler (Async): {0}, {1}", machine.GetType().Name, initialEvent != null ? initialEvent.GetType().Name : string.Empty);
            try
            {
                if (isFresh)
                {
        ...
        internal override bool GetNondeterministicBooleanChoice(AsyncMachine machine, int maxValue)
        {
            ...
            bool result = NekaraApi.CreateNondetBool();
        ...
        internal override int GetNondeterministicIntegerChoice(AsyncMachine machine, int maxValue)
        {
            ...
            int result = NekaraApi.CreateNondetInteger(maxValue);
        ...
        {
            NekaraApi.Assert(predicate, string.Empty);
        }
        ...
        {
            // Console.WriteLine("\n>>> Invoked Machine Action >>> {0}", action.Name.ToString());
            if (this.Configuration.IsVerbose)
            {
