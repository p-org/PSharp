//-----------------------------------------------------------------------
// <copyright file="TryStatementVisitor.cs">
...
{
    /// <summary>
    /// The P# try statement parsing visitor.
    /// </summary>
    internal sealed class TryStatementVisitor : BaseParseVisitor
    {
        /// <summary>
        /// Constructor.
        /// </summary>
        /// <param name="tokenStream">TokenStream</param>
        internal TryStatementVisitor(TokenStream tokenStream)
            : base(tokenStream)
        {
        ...
        internal void Visit(BlockSyntax parentNode)
        {
            ...
            while (!base.TokenStream.Done &&
                base.TokenStream.Peek().Type == TokenType.Catch)
            {
                node.CatchKeywords.Add(base.TokenStream.Peek());
                ...
                {
                    node.CatchLeftParenthesisTokens.Add(base.TokenStream.Peek());
                    ...
                    if (expr.StmtTokens.Any(tok => tok.Type == TokenType.NonDeterministic) &&
                        expr.StmtTokens.Count > 1)
                    {
                        throw new ParsingException("Unexpected catch expression.",
                            new List<TokenType>());
                    }

                    node.CatchExpressions.Add(expr);
                    ...
                    node.CatchRightParenthesisTokens.Add(base.TokenStream.Peek());
                ...
                if (base.TokenStream.Peek().Type == TokenType.New)
                {
                    new NewStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.CreateMachine)
                {
                    new CreateStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.RaiseEvent)
                {
                    new RaiseStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.SendEvent)
                {
                    new SendStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.Monitor)
                {
                    new MonitorStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.PushState)
                {
                    new PushStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.Pop)
                {
                    new PopStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.Assert)
                {
                ...
                {
                    new WhileStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.ForLoop)
                {
                    new ForStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.ForeachLoop)
                {
                    new ForeachStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.Lock)
                {
                    new LockStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.Try)
                {
                    new TryStatementVisitor(base.TokenStream).Visit(blockNode);
                    ...
                    base.TokenStream.Peek().Type == TokenType.Await)
                {
                    new GenericStatementVisitor(base.TokenStream).Visit(catchBlockNode);
                }
                else if (base.TokenStream.Peek().Type == TokenType.LeftCurlyBracket)
                {
                ...
                node.CatchStatementBlocks.Add(catchBlockNode);
            }

            if (base.TokenStream.Peek().Type == TokenType.Finally)
            {
                node.FinallyKeyword = base.TokenStream.Peek();
                ...
                node.FinallyStatementBlock = catchBlockNode;
            }
