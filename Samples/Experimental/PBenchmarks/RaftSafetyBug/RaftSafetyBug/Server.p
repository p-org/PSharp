...
machine Server
{
	...
	state Leader
	{
		entry
		{
            ...
            announce NotifyLeaderElected, CurrentTerm;
	...
	fun BroadcastVoteRequests()
	{
		...
		send PeriodicTimer, PeriodicTimer_StartTimer;
...
spec SafetyMonitor observes NotifyLeaderElected
{
	...
	start state Init
	{
		...
		{
			CurrentTerm = -1;
            TermsWithLeader = default(seq[int]);
            raise LocalEvent;
		}
	...
	state Monitoring
	{
		on NotifyLeaderElected do (term : int)
		{
			var index : int;

			index = 0;
			while(index < sizeof(TermsWithLeader))
			{
				if(term == TermsWithLeader[index])
				{
					assert false;
				}
				index = index + 1;
			}
            TermsWithLeader += (sizeof(TermsWithLeader), term);
		}
	}
}