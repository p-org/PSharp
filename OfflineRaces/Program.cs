...
namespace OfflineRaces
{
    ...
    public class MAccess
    {
        ...
        public MAccess(bool isWrite, UIntPtr location, UIntPtr objHandle, UIntPtr offset, string srcLocation, int machineId)
        {
            this.isWrite = isWrite;
            this.location = location;
            this.objHandle = objHandle;
    ...
    internal class CreateTask : Node
    {
        ...
        public CreateTask(int taskId, int machineId)
        {
            this.taskId = taskId;
            this.machineId = machineId;
        }
    ...
    public class AgNode
    {
        ...
        public int uniqId;
        public int machineId;
        public List<MAccess> addresses = new List<MAccess>();
    }
    ...
    internal class AgEdge : QuickGraph.IEdge<AgNode>
    {
        public AgNode src;
        public AgNode trg;
        public AgEdge(AgNode src, AgNode trg)
        {
            this.src = src;
            this.trg = trg;
        }

        AgNode IEdge<AgNode>.Source
        {
            get
            {
                return src;
    ...
    public class Program
    {
        ...
        //Aggergate node graph
        static QuickGraph.AdjacencyGraph<Node, Edge> AgGraph = new AdjacencyGraph<Node, Edge>();
            ...
            foreach (MachineTrace mt in machineTrace)
            {
                if (!mt.isSend && (mt.actionID != 0) && !mt.isTaskMachine)
                {
                    ...
                    foreach (ActionInstr ins in matching.accesses)
                    {
                        ...
                        {
                            nd1 = new CreateTask(ins.taskId, mt.machineID);
                        }
        ...
        static void compressGraph()
        {
            ...
            Stack<cNode> dfsA = new Stack<cNode>();

            foreach (Node v in HbGraph.Vertices)
            {
                if (v.GetType().ToString().Contains("ActBegin"))
                {
                    dfsS.Push(v);
                    cNode an = new cNode();
                    AgGraph.AddVertex(an);
                    an.mcId = ((ActBegin)v).machineID;
                    dfsA.Push(an);
                }
            }
            ...
            while (dfsS.Count > 0)
            {
                ...
                if(outEdges.Count() == 1)
                {
                    foreach (Edge outEdge in outEdges)
                    {
                        ...
                        if (successor.GetType().ToString().Contains("MemAccess"))
                        {
                            MemAccess m = (MemAccess)successor;
                            av.addresses.Add(new MemAccess(m.isWrite, m.location, m.objHandle, m.offset, m.srcLocation, m.machineId));
                ...
                else
                {
                    ...
                    {
                        Node successor = outEdge.trg;
                        AgGraph.AddEdge(new Edge(av, successor));
        ...
        /*static void newGraph()
        {
            ...
            Stack<Node> dfsS = new Stack<Node>();
            Stack<AgNode> dfsA = new Stack<AgNode>();
            ...
            foreach(Node v in HbGraph.Vertices)
            {
                ...
                {
                    dfsS.Push(v);
                    AgNode an = new AgNode();
                    an.machineId = ((ActBegin)v).machineID;
                    AgGraph.AddVertex(an);
                    dfsA.Push(an);
                }
            }
            ...
            while (dfsS.Count > 0)
            {
                ...
                if(outEdges.Count() == 1)
                {
                    foreach (Edge outEdge in outEdges)
                    {
                        ...
                        {
                            MemAccess m = (MemAccess)successor;
                            av.addresses.Add(new MAccess(m.isWrite, m.location, m.objHandle, m.offset, m.srcLocation, m.machineId));
                            if (!successor.visited)
                            {
                                dfsS.Push(successor);
                                dfsA.Push(av);
                        ...
                        else
                        {
                            ...
                            {
                                dfsS.Push(successor);
                                AgNode agn = new AgNode();
                                dfsA.Push(agn);
                                AgGraph.AddVertex(agn);
                                AgGraph.AddEdge(new AgEdge(agn, av));
                                if (successor.GetType().ToString().Contains("SendEvent"))
                                {
                                    agn.machineId = ((SendEvent)successor).toMachine;
                                }
                            }
                        }       
                    }
